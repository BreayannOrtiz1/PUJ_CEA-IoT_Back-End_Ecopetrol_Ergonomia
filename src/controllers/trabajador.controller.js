...
import * as service from '../services/trabajador.service.js';
...
export async function register(req, res) {
    // Crea un nuevo  (valores ya validados por Joi)
    console.log("Controlador - Datos recibidos:", req.body);
    const created = await service.create(req.body);
    res.status(201).json({ ok: true, message: 'Trabajador registrado', data: created });
}
...
export async function update(req, res) {
    ...
    res.json({ ok: true, message: 'Trabajador actualizado', data: updated });
}
...
export async function remove(req, res) {
    ...
    res.json({ ok: true, message: 'Trabajador eliminado', data: delJson });
}
