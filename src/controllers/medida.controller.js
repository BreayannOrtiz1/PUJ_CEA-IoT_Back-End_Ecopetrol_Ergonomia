...
export async function register(req, res) {
    // Crea un nuevo sensor (valores ya validados por Joi)
    console.log("Controlador - Datos recibidos:", req.body);
    const created = await service.create(req.body);
    res.status(201).json({ ok: true, message: 'Sensor registrado', data: created });
}


export async function update(req, res) {
    // Actualiza sensor existente, si no existe -> 404
    console.log("Controlador - Datos recibidos para actualizar:", req.body);
    const updated = await service.update(req.body);
    res.json({ ok: true, message: 'Sensor actualizado', data: updated });
}
...
export async function remove(req, res) {
    ...
    res.json({ ok: true, message: 'Sensor eliminado', data: delJson });
}
